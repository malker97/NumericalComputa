...
// 'maxreps' is the maximum iterations through the loop
{
  ...
  // your code goes here!
  if(left > right){
    printf("left > right, please check your input");
    return 0;
  }
  ...
  if(f(left) * f(mid) < 0){
    return interval_bisection(f, left, mid, epsilon, maxreps - 1, results);
  }
  else{
    return interval_bisection(f, mid, right, epsilon, maxreps - 1, results);
  }
  // return 0 failure, else 'numreps' (>= 1)
